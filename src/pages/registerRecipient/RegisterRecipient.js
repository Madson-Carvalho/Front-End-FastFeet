    ...
    useEffect(() => {
        if (isEditMode) {
            fetch(`http://localhost:3333/api/v1/recipient/find-by-id/${id}`, {
                ...
                .catch(error => console.error('Erro ao buscar Destinatário:', error));
        }
    }, [id, isEditMode]);
    ...
    const handleSubmit = (e) => {
        ...
        fetch(postEndpoint, {
            ...
            .then(response => {
                ...
                toast.success(`Destinatário ${action} com sucesso!`);
                setTimeout(() => {
                    navigate('/list-recipient');
                }, 3000);
            })
            .catch(e => toast.error(`Erro ao ${isEditMode ? 'atualizar' : 'cadastrar'} Destinatário`, e))
    }

    return (
        <>
            <Header />
            <Main title={isEditMode ? "Editar Destinatário" : "Criar Destinatário"}>
                <form className='base-form' onSubmit={handleSubmit}>
                    <CustomInput id='name' type='text' name='name' label='Nome' placeholder=" " value={recipientData.name} onChange={handleChange}
                        required={true} />
                    <CustomInput id='cpf' type='text' name='cpf' label='CPF' placeholder=" " value={recipientData.cpf} onChange={handleChange}
                        required={true} />
                    <CustomInput id='phone' type='phone' name='phone' label='Telefone' placeholder=" "
                        value={recipientData.phone} onChange={handleChange} />
                    <CustomInput id='email' type='email' name='email' label='E-mail' placeholder=" "
                        value={recipientData.email} onChange={handleChange} required={true} />
                    <CustomInput id='address' type='text' name='address' label='Endereço' placeholder=" "
                        value={recipientData.address} onChange={handleChange} required={true} />
                    <CustomSelect id='uf' name='uf' label='Estado' options={ufs} onChange={handleChange}
                        value={recipientData.uf} required={true} />
                    <CustomSelect id='city' name='city' label='Cidade' options={cities} onChange={handleChange}
                        value={recipientData.city} required={true} />
